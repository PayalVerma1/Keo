...
export default function GithubChecksPage() {
  ...
  const fetchScores = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to load GitHub checks");
      setData((await response.json()) as DashboardPayload);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load GitHub checks");
    } finally {
      setLoading(false);
    }
  }, [router]);
        ...
        <div className="dashboard-scroll-area">
          <div className="page-hero">
            <div className="page-title-wrap">
              <h1 className="page-title">GitHub checks</h1>
              <p className="page-subtitle">
                Optional last mile. Same fingerprint engine as the MCP tool, posted on a pull request.
              </p>
            </div>
          </div>
